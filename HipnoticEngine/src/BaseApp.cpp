...
BaseApp::init() {
  ...
  hr = m_shaderProgram.init(m_device, "HipnoticEngine.fx", Layout);
  if (FAILED(hr)) {
    return hr;
  }
...
BaseApp::update() {
  ...
  ImGui::Begin("Killer Klown");
  ImGui::End();

  m_userInterface.Inspector();
  ...
  m_modelMtrix = scaleMatrix * rotationMatrix * traslationMatrix;

  // Update variables that change once per frame
  cb.mWorld = XMMatrixTranspose(m_modelMtrix);
